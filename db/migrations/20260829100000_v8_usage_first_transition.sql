-- v8 — SPEND FOLLOWS THE WORK THAT EARNED IT, not the work it finished on.
--
-- What the first Hermes run measured (2026-08-29, PR #146). An external operator drove a
-- full delivery hands-off. Every family's spend was captured — and one capability read
-- `unknown` anyway:
--
--   grok+grok-4.6  role:reviewer     1 delivered   52,536 tokens
--   grok+grok-4.6  role:integrator   1 delivered   unknown
--
-- The timeline says why. ONE sweep, one actor, two claims:
--
--   18:02:04  transition  integrating → validating   "merged"
--   18:03:54  transition  validating  → done         "green on main"
--   18:04:05  usage       52,536
--
-- api.family_track_record resolved a usage event's capability as the family's MOST RECENT
-- transition on that task at or before the event. The most recent was validating→done,
-- which requires role:reviewer. So the integrator's whole cost was booked to the reviewer,
-- and role:integrator was left with no usage row at all.
--
-- Why that is worse than a gap. M20's contract (design/track-record.md D3) is that an
-- unmeasured family reads `unknown`, NEVER `0` — unknown ≠ free. Here `unknown` was
-- neither: the spend existed, was recorded, and was filed under a neighbour. The signal
-- said "we did not measure this" when the truth was "we measured it and attributed it
-- wrong", and nothing in the view distinguishes the two. It also inflates the neighbour:
-- role:reviewer's tokens_per_delivery carried a merge it did not perform.
--
-- The fix. Charge the FIRST role-bearing transition of the sweep instead of the last, with
-- the sweep window bounded below by the family's previous usage event on that task (usage
-- is reported once per sweep, so the previous report is exactly where this sweep begins).
-- A single-transition sweep — every other sweep in that run — is unaffected: first and
-- last are the same row.
--
-- View-only. No table, no verb, no grant changes: api.record_usage still writes the same
-- event, the driver still reports once per sweep, and the ledger from the empty-sweep
-- work is untouched. This is a reading of existing history, so it corrects the PAST too —
-- the run above re-reads correctly the moment this lands.
