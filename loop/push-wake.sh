...
wake_listen_stream() {
  ...
  { echo "listen $WAKE_CHANNEL;"; while :; do echo "select pg_sleep(0.5);"; done; } \
    ...
    | awk '/Asynchronous notification/ { print "wake"; fflush() }' || true
  return 0
}
...
idle_wait() {
  ...
  if [ "${LOOP_PUSH_WAKE:-1}" != "1" ] || [ "${WAKE_ACTIVE:-0}" != "1" ]; then
  ...
  # the sleep branch above (poll this tick; re-establish on the next).
  ...
  # Otherwise wait on the FIFO for up to <secs>. Capture read's rc DIRECTLY: after
  # `if read; then ...; fi`, bash 3.2's $? is 0 (the if-compound), not read's status,
  # so a timeout would be misread as a stream error and disable push-wake.
  # bash 3.2 `read -t` on a FIFO returns 1 after the wait (not >128 as in bash 4+);
  # distinguish that backstop from an immediate error by elapsed time.
  local line rc t0 elapsed
  rc=0
  t0="$(date +%s)"
  read -r -t "$secs" line <&9 2>/dev/null || rc=$?
  elapsed=$(( $(date +%s) - t0 ))
  if [ "$rc" -eq 0 ]; then
    # A notification arrived. COALESCE: drain the burst so it becomes ONE wake.
    while read -r -t "$WAKE_DRAIN_SECS" _ <&9; do :; done
    if [ -n "$reason" ]; then
  ...
  # Timeout: bash 4+ reports >128; bash 3.2 reports 1 after the wait. An immediate
  # failure (closed fd) has elapsed 0 — that is the broken stream, not the backstop.
  if [ "$rc" -gt 128 ] || [ "$elapsed" -ge 1 ]; then
    return 0
  fi
  # Immediate non-zero → broken stream. Log, WAKE_ACTIVE=0, return 0.
