...
RUN_DIR="$TMP"
WAKE_FIFO="$TMP/wake.fifo"
export RUN_DIR WAKE_FIFO
...
# Stub LISTEN streams. Quoted heredocs so nothing expands at write time.
cat > "$TMP/wake-soon.sh" <<'STUB'
#!/usr/bin/env bash
sleep 1
echo dev
STUB
cat > "$TMP/silent.sh" <<'STUB'
#!/usr/bin/env bash
sleep 300
STUB
cat > "$TMP/burst.sh" <<'STUB'
#!/usr/bin/env bash
sleep 1
printf 'dev\ndev\ndev\ndev\ndev\n'
STUB
cat > "$TMP/broken.sh" <<'STUB'
#!/usr/bin/env bash
exit 1
STUB
chmod +x "$TMP"/wake-soon.sh "$TMP"/silent.sh "$TMP"/burst.sh "$TMP"/broken.sh
...
reset_listener "$TMP/wake-soon.sh"
t0="$(date +%s)"
...
#    A timeout must also stay SILENT and leave WAKE_ACTIVE=1 (not misread as a stream
#    error — bash 3.2 loses read's rc after `if read; then; fi`).
reset_listener "$TMP/silent.sh"
t0="$(date +%s)"
idle_wait 3 >"$TMP/idle.out" 2>&1
d="$(elapsed "$t0")"
...
if [ "$WAKE_ACTIVE" = "1" ] && ! grep -q DEGRADE "$TMP/idle.out"; then
  pass "backstop silent: timeout left WAKE_ACTIVE=1 and emitted no DEGRADE"
else
  fail "backstop silent: WAKE_ACTIVE=$WAKE_ACTIVE log=$(cat "$TMP/idle.out")"
fi

# 3. COALESCING — a burst collapses to ONE wake, not five spurious ones.
reset_listener "$TMP/burst.sh"
t0="$(date +%s)"
idle_wait 30 >/dev/null 2>&1
d1="$(elapsed "$t0")"
t0="$(date +%s)"
